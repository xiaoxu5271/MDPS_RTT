...
#define UDP_SERVER_IP "192.168.1.107"
#endif
//lch add end
...
static int send_http_get(const char* uri,unsigned char *buf,int *len)
{
    ...
    do
    {
        length = webclient_read(session, buffer_read, file_size - total_length > GET_REV_BUF_SIZE ?
                            GET_REV_BUF_SIZE : file_size - total_length);   
        if (length > 0)
        {
